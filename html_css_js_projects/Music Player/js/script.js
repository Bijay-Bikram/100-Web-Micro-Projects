...
async function getSonggs(folder) {
    currFolder = folder
...
async function displayAlbums() {
    ...
    for (let index = 0; index < anchors.length; index++) {
        ...
        if (element.href.includes("/songs/") && !element.href.includes(".htaccess")) {
            ...
            let a = await fetch(`http://127.0.0.1:5500/songs/bossa/info.json`)
            let resp = await a.json()

            songContainer.innerHTML = songContainer.innerHTML + `  <div class="card" data-folder="bossa">
                    <img src="./songs/bossa/cover.jpg" alt="">
                    <div class="songInfo">
                        <h3>${resp.title}</h3>
